...
module.exports = async (req, res) => {
	try {
		...
		// validando datos
		const valid_data = await Cat_subtipo_productos.findAll({ where: { nombre } });
		...
		if (nombre && valid_nombre) {
			await Cat_subtipo_productos.update(
				{ nombre: nombre },
				{
					where: { id_subtipo_producto: id },
				}
			);
		}
		...
		// respondemos
		const resps = await Cat_subtipo_productos.findAll({ where: { id_subtipo_producto: id } });
