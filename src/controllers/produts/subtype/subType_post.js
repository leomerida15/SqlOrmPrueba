...
module.exports = async (req, res) => {
	try {
		...
		// validando nombre
		const valid_name = await Cat_subtipo_productos.findAll({ where: { nombre } });
		if (valid_name.length > 0) throw { message: 'Ese subtipo de producto ya exite', code: 400 };

		// guardamos
		const data = await Cat_subtipo_productos.create({ nombre, sn_activo: true });

		res.status(200).json({ message: 'SubTipo de producto Editado', data: data.dataValues });
	} catch (err) {
		error(req, res, err);
	}
};
